...
RSpec.describe AccessTokensController, type: :controller do
  describe '#create' do
    let(:params) do
      {
        data: {
          attributes: { login: 'emanuele', password: 'secret' }
        }
      }
    end

		context 'when no auth_data code is provided' do
			subject { post :create }
    ...
    context 'when invalid password provided' do
      let(:user) { create :user, login: 'emanuele', password: 'invalid' }
      subject { post :create, params: params }
      before { user }
    ...
    context 'when valid credentials provided' do
      ...
      it 'should return proper json body' do
        subject
				hash = { json_data['attributes'].keys[1] => json_data['attributes'].values[1] }
				expect(hash).to eq(
		...
		context 'when success request' do
			...
			it 'should return proper json body' do
				...
				user = User.find_by(login: 'emanuele')
				hash = { json_data['attributes'].keys[1] => json_data['attributes'].values[1] }
				expect(hash).to eq(
					{ 'token' => user.access_token.token }
				)
